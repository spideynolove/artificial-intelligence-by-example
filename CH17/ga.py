...
def get_fitness(this_choice, scenario):
    if (scenario == 1):
        fitness = sum(1 for expected, actual in zip(
            target, this_choice) if expected == actual)
    if (scenario == 0):
...
def crossover(parent):
    # producing a random position of the parent gene
    index = random.randrange(0, len(parent))
    childGenes = list(parent)
    oldGene = childGenes[index]        # for diversity check
    newGene, alternate = random.sample(geneSet, 2)
    if (newGene != oldGene):
        childGenes[index] = newGene  # natural crossover
    if (newGene == oldGene):
        # mutation introduced to ensure diversity to avoid to get stuck in a local minima
...
def display(selection, bestFitness, childFitness, startTime):
    ...
    print("Selection:", selection, "Fittest:", bestFitness,
          "This generation Fitness:", childFitness, "Time Difference:", timeDiff)
...
# II. SUBSEQUENT GENERATIONS
    ...
    g = 0
    # at the beginning, the first parent, best or not is our best available choice
    bestParent = alphaParent
    while True:
        g += 1
        ...
        bestParent = child
        if scenario == 1:
            goal = len(alphaParent)  # number of good genes=parent length
        if scenario == 0:
            goal = threshold
