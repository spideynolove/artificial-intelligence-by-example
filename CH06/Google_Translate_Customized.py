...
phrase_translation = ['', '', '', 'Il est agressif',
                      '', '', '', '', '', '', '', '']
V1 = ['broke', 'road', 'stopped', 'shouted', 'coach', 'bus',
      'car', 'truck', 'break', 'broke', 'roads', 'stop']
...
def g_translate(source, targetl, m):
    ...
    request = service.translations().list(
        q=source, target=targetl, format='text', model=m)
...
t = 0
f = 0
dt = 0     # true;false;deeper translation call
for xi in range(len(X)):
    ...
    if (source == back_translate):
        ...
        if (X2[xi] > 0):
            ...
            dt += 1
    if (f > 0):
        # error detection probablity before deep translation
        B1 = frequency_p(xi+1, f)
        # error detection probablity after deep translation
        B2 = frequency_p(xi+1, f-dt)
    if (f > 0):
...
print('Customized Google Translate:', "True:", t,
      "False:", f-dt, 'ETP', round((f-dt)/len(X), 2))
a = 2.5
at = t+a
af = f-a  # subjective acceptance of an approximate result
print('Google Translate acceptable:', "True:", at,
      "False:", af, 'ETP', round(af/len(X), 2))
# The error rate should decrease and be stablized as the KNN knowledge base increases
print('Customized Google Translate acceptable:', "True:",
      at, "False:", af-dt, 'ETP', round((af-dt)/len(X), 2))
